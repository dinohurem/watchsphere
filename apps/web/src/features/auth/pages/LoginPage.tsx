...
// Mirrors WHATSAPP_OTP_RESEND_COOLDOWN_SECONDS on the server.
...
export function LoginPage() {
  ...
  // Two login modes, matching the mobile app: the existing email + password,
  // and passwordless sign-in with a code delivered over WhatsApp.
  ...
  const handleSendWhatsappCode = async () => {
    ...
    // The server answers identically for unknown numbers so it never leaks
    // whether an account exists — this wording must not imply that it does.
  ...
  const switchMode = (next: 'password' | 'whatsapp') => {
    setMode(next);
    setError('');
    setNotice('');
    setCodeSent(false);
          ...
          <div className="w-full max-w-[450px] flex flex-col gap-6 sm:gap-8">
            ...
            <div className="flex p-1 bg-[rgba(29,29,31,0.06)] rounded-full">
              ...
              <button
                ...
                className={`flex-1 h-[36px] rounded-full text-[14px] font-semibold tracking-[0.075px] transition-colors ${
                  mode === 'whatsapp' ? 'bg-white text-[#1d1d1f] shadow-sm' : 'text-[rgba(29,29,31,0.6)]'
                }`}
              >
                {t('auth.login.modeWhatsapp')}
              </button>
            </div>
              ...
              <div className="flex flex-col gap-4">
                {mode === 'password' ? (
                  ...
                  <>
                    ...
                    {codeSent && (
                      <div className="flex flex-col gap-1.5">
                        <label htmlFor="whatsapp_code" className="text-[15px] font-semibold text-[#1d1d1f] leading-5 tracking-[0.075px]">
