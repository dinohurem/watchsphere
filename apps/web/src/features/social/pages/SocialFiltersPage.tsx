...
export function SocialFiltersPage() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();

  // Fetch dynamic social filters from backend
  const { data: socialFilters = [], isLoading: isLoadingFilters } = useSocialFilters();
  ...
  // Sort filters by display order
  const sortedFilters = useMemo(() => {
    return [...socialFilters].sort((a, b) => a.display_order - b.display_order);
  }, [socialFilters]);

  // Get currently selected filter
  const selectedFilter = useMemo(() => {
    if (!selectedFilterKey) return null;
    return socialFilters.find(f => f.key === selectedFilterKey) || null;
  }, [selectedFilterKey, socialFilters]);
  ...
  const handleSave = () => {
    ...
    // Navigate back to social search with filters
    navigate(`/app/social-search${params.toString() ? `?${params.toString()}` : ''}`);
  };
