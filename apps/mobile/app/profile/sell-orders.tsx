...
export default function SellOrdersScreen() {
  ...
  const loadOrders = async () => {
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        const sellOrdersData: Order[] = response.data
          .filter((order: any) => order.order_type === 'sell')
          .map((order: any) => ({
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Sell Orders</Text>
